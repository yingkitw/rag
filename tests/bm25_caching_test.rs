use rag::{embeddings::EmbeddingModel, retriever::Retriever, vector_store::InMemoryVectorStore};
...
async fn bm25_cache_avoids_rebuild() {
    ...
    retriever
        .add_document("rust systems programming memory safety".to_string())
        .await
        .unwrap();
    retriever
        .add_document("python scripting dynamic typing easy".to_string())
        .await
        .unwrap();
    ...
    retriever
        .add_document("javascript web frontend".to_string())
        .await
        .unwrap();
    let r3 = retriever
        .retrieve_hybrid("web frontend", 0.7)
        .await
        .unwrap();
    assert!(!r3.is_empty());
}
