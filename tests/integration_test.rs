use rag::{
    ...
    retriever::Retriever,
    vector_store::{Document, InMemoryVectorStore, VectorStore, cosine_similarity},
};
...
async fn test_vector_store() {
    ...
    let doc = Document::new("test content".to_string()).with_embedding(vec![0.1, 0.2, 0.3]);
...
async fn test_document_creation() {
    let doc =
        Document::new("content".to_string()).with_metadata("key".to_string(), "value".to_string());
...
async fn test_vector_store_search() {
    ...
    let doc3 = Document::new("rust development".to_string()).with_embedding(vec![0.9, 0.1, 0.0]);
